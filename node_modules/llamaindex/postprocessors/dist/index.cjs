...
class MetadataReplacementPostProcessor {
    ...
    async postprocessNodes(nodes) {
        for (const n of nodes){
            n.node.setContent(n.node.metadata[this.targetMetadataKey] ?? n.node.getContent(schema.MetadataMode.NONE));
        }
        return nodes;
    }
}
...
class JinaAIReranker {
    constructor(init){
        ...
        this.apiKey = env.getEnv("JINAAI_API_KEY");
        if (!this.apiKey) {
    ...
    async postprocessNodes(nodes, query) {
        ...
        const documents = nodes.map((n)=>n.node.getContent(schema.MetadataMode.ALL));
        const results = await this.rerank(utils.extractText(query), documents, this.topN);
        const newNodes = [];
...
exports.JinaAIReranker = JinaAIReranker;
