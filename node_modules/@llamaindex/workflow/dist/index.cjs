...
var run = require('@llama-flow/core/stream/run');
...
const agentToolCallEvent = core.workflowEvent();
const agentToolCallResultEvent = core.workflowEvent();
const agentInputEvent = core.workflowEvent();
const agentSetupEvent = core.workflowEvent();
const agentStreamEvent = core.workflowEvent();
const agentOutputEvent = core.workflowEvent();
...
const startAgentEvent = core.workflowEvent({
    debugLabel: "llamaindex-start"
});
const stopAgentEvent = core.workflowEvent({
    debugLabel: "llamaindex-stop"
});
const toolCallsEvent = core.workflowEvent();
const toolResultsEvent = core.workflowEvent();
const agentStepEvent = core.workflowEvent();
/**
 ...
 */ class AgentWorkflow {
    constructor({ agents, rootAgent, verbose }){
        ...
        this.workflow = this.stateful.withState(core.createWorkflow());
        this.agents = new Map();
        ...
        this.executeToolCalls = async (event)=>{
            const { sendEvent } = core.getContext();
    ...
    createInitialState() {
        return {
            memory: new memory.ChatMemoryBuffer({
                llm: this.agents.get(this.rootAgentName)?.llm ?? global.Settings.llm
            }),
            scratchpad: [],
            currentAgentName: this.rootAgentName,
